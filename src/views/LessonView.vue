...
type StepKind = 'theory' | 'quiz' | 'code' | 'markup' | 'dom' | 'sql' | 'blank' | 'order'
...
const coach = computed<{ mood: 'idle' | 'happy' | 'thinking'; message: string }>(() => {
  ...
  switch (currentStep.value?.kind) {
    ...
    case 'sql':
      return { mood: 'thinking', message: 'Напиши SQL-запрос и запусти — сверю с базой 🗄️' }
    case 'blank':
  ...
  <div v-if="flat" class="lesson container">
    ...
    <template v-else>
      ...
      <div class="lesson__body card">
        ...
        />
        <SqlStep
          v-else-if="currentStep.kind === 'sql'"
          :key="currentIndex"
          :step="currentStep"
          @solved="onCodeSolved"
        />
